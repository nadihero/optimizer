...
export default function BudgetPage() {
  ...
  const handleAddBudget = async (e: React.FormEvent) => {
    ...
    await addBudget({ category, limit_amount: Number(limitAmount.replace(/\./g, "")), spent_amount: 0, period, icon: "📊", color: "#D4F53C" });
    setShowForm(false);
    setCategory("");
    setLimitAmount("");
  };
  ...
  const handleEditBudget = async (id: string) => {
    if (!category || !limitAmount) return;
    await updateBudget(id, { category, limit_amount: Number(limitAmount.replace(/\./g, "")), period });
    setEditingId(null);
    setShowForm(false);
    setCategory("");
    setLimitAmount("");
  ...
  const startEdit = (budget: typeof budgets[0]) => {
    setEditingId(budget.id);
    setShowForm(true);
    setCategory(budget.category);
    setLimitAmount(String(budget.limit_amount).replace(/\B(?=(\d{3})+(?!\d))/g, "."));
      ...
      <main className="px-5 pb-28">
        ...
        {showForm && (
          <form onSubmit={(e) => { e.preventDefault(); editingId ? handleEditBudget(editingId) : handleAddBudget(e); }} className="bg-white dark:bg-darkCard rounded-2xl p-5 shadow-[0_2px_8px_rgba(0,0,0,0.04)] dark:shadow-none dark:border dark:border-gray-700 mb-6">
            ...
            <div className="mb-4">
              <label className="text-sm text-grayText dark:text-gray-400 mb-2 block">Kategori</label>
              <input type="text" value={category} onChange={(e) => setCategory(e.target.value)} placeholder="Contoh: Makanan" className="w-full text-sm text-dark dark:text-gray-100 py-2 bg-grayLight dark:bg-dark rounded-xl px-4 outline-none focus:ring-2 focus:ring-lime transition" required />
            </div>
            <div className="mb-4">
              <label className="text-sm text-grayText dark:text-gray-400 mb-2 block">Limit</label>
              <input type="text" value={limitAmount} onChange={(e) => setLimitAmount(formatInputAmount(e.target.value))} placeholder="0" className="w-full text-sm text-dark dark:text-gray-100 py-2 bg-grayLight dark:bg-dark rounded-xl px-4 outline-none focus:ring-2 focus:ring-lime transition" inputMode="numeric" required />
            </div>
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => { setShowForm(false); setEditingId(null); setCategory(""); setLimitAmount(""); }} className="flex-1 py-3 bg-grayLight dark:bg-dark rounded-xl font-medium text-grayText hover:bg-grayMid dark:hover:bg-gray-700 transition">Batal</button>
              <button type="submit" className="flex-1 py-3 bg-lime rounded-xl font-medium text-dark hover:bg-limeDark transition">{editingId ? "Update" : "Simpan"}</button>
            </div>
          </form>
        )}
                  ...
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="hidden group-hover:flex gap-1">
                        <button onClick={() => startEdit(budget)} className="p-1 hover:bg-grayLight dark:hover:bg-gray-700 rounded-full"><svg className="w-4 h-4 text-iosBlue" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                        <button onClick={() => { if (confirm("Hapus budget ini?")) deleteBudget(budget.id); }} className="p-1 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full"><svg className="w-4 h-4 text-expense" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                      </div>
                    </div>
                  </div>
