    ...
    <>
      <header className="px-5 pt-12 pb-4">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h1 className="text-xl font-bold text-dark dark:text-gray-100">Semua Transaksi</h1>
        </div>
        <div className="flex bg-grayLight dark:bg-darkCard rounded-full p-1">
          <button onClick={() => setFilter("all")} className={`flex-1 py-2 rounded-full text-xs font-medium transition ${filter === "all" ? "bg-white dark:bg-dark shadow-sm text-dark dark:text-gray-100" : "text-grayText"}`}>Semua</button>
          <button onClick={() => setFilter("income")} className={`flex-1 py-2 rounded-full text-xs font-medium transition ${filter === "income" ? "bg-white dark:bg-dark shadow-sm text-income" : "text-grayText"}`}>Masuk</button>
          <button onClick={() => setFilter("expense")} className={`flex-1 py-2 rounded-full text-xs font-medium transition ${filter === "expense" ? "bg-white dark:bg-dark shadow-sm text-expense" : "text-grayText"}`}>Keluar</button>
        </div>
      </header>
      <main className="px-5 pb-28">
        {Object.keys(groupedTransactions).length === 0 ? (
          <div className="text-center py-16 text-grayText dark:text-gray-400"><p className="text-sm">Tidak ada transaksi</p></div>
        ) : (
            ...
            return (
              <div key={date} className="mb-6">
                <div className="flex justify-between items-center mb-3">
                  <h3 className="text-sm font-semibold text-dark dark:text-gray-100">{formatDateShort(date)}</h3>
