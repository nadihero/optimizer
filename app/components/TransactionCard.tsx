...
export default function TransactionCard({ id, name, date, amount, type, iconColor }: TransactionCardProps) {
      ...
      <div className="p-4 glass-card">
        <div className="space-y-3">
          <input
            ...
            className="w-full text-sm text-gray-100 py-2 bg-white/10 rounded-lg px-3 outline-none border border-white/10"
            placeholder="Nama"
          />
          <input
            type="text"
          ...
          <div className="flex gap-2">
            <button onClick={() => setEditing(false)} className="flex-1 py-2 bg-white/10 rounded-lg text-sm text-gray-400">Batal</button>
            <button onClick={handleSave} className="flex-1 py-2 rounded-lg text-sm font-medium text-white" style={{ background: "var(--main-gradient)" }}>Simpan</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div
      className="flex items-center justify-between p-4 glass-card transition duration-200 hover:-translate-y-0.5 active:scale-[0.98] cursor-pointer"
      onClick={() => setShowActions(!showActions)}
    >
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: `${iconColor}20` }}>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" color={iconColor} fill="none" stroke={iconColor} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
            {isIncome ? (
      ...
      <div className="flex items-center gap-2">
        <span className={`font-semibold text-sm ${isIncome ? "text-income" : "text-gray-100"}`}>
          {isIncome ? "+" : "-"}{amount}
        </span>
        {showActions && (
          <div className="flex gap-1">
            ...
            <button
              ...
              className="p-1.5 hover:bg-red-500/20 rounded-full transition"
            >
