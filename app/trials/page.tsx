    ...
    <>
      <header className="px-5 pt-12 pb-4">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-3"><button onClick={() => router.back()} className="w-10 h-10 glass-card flex items-center justify-center"><svg className="w-5 h-5 text-gray-100" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg></button><h1 className="text-xl font-bold text-gray-100">Trials</h1></div>
          <button onClick={() => { setShowForm(!showForm); setEditingId(null); setName(""); setStartDate(""); setEndDate(""); setWillConvert(false); setAmount(""); }} className="w-10 h-10 rounded-full flex items-center justify-center text-white" style={{ background: "var(--main-gradient)" }}><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 5v14m-7-7h14" /></svg></button>
        </div>
      </header>
      <main className="px-5 pb-28">
        {showForm && (
          <form onSubmit={(e) => { e.preventDefault(); editingId ? handleEditTrial(editingId) : handleAddTrial(e); }} className="glass-card p-5 mb-6">
            <h3 className="text-base font-bold text-gray-100 mb-4">{editingId ? "Edit Trial" : "Tambah Trial"}</h3>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div><label className="text-sm text-gray-400 mb-2 block">Mulai</label><input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full text-sm text-gray-100 py-2 bg-white/10 rounded-xl px-4 outline-none border border-white/10 focus:border-lime transition" /></div>
              <div><label className="text-sm text-gray-400 mb-2 block">Berakhir</label><input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="w-full text-sm text-gray-100 py-2 bg-white/10 rounded-xl px-4 outline-none border border-white/10 focus:border-lime transition" required /></div>
            </div>
            <div className="mb-4"><label className="text-sm text-gray-400 mb-2 block">Harga setelah trial (opsional)</label><input type="text" value={amount} onChange={(e) => setAmount(formatInputAmount(e.target.value))} placeholder="0" className="w-full text-sm text-gray-100 py-2 bg-white/10 rounded-xl px-4 outline-none border border-white/10 focus:border-lime transition" inputMode="numeric" /></div>
            <div className="mb-4"><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={willConvert} onChange={(e) => setWillConvert(e.target.checked)} className="w-4 h-4 rounded accent-lime" /><span className="text-sm text-gray-100">Akan berlangganan otomatis</span></label></div>
        ...
        <div className="space-y-3">
          {trials.length === 0 ? (<div className="text-center py-16 text-gray-400"><p className="text-sm">Belum ada trial</p><p className="text-xs mt-1">Tap + untuk menambahkan</p></div>) : (
            trials.map((trial) => {
              const daysLeft = getDaysRemaining(trial.end_date);
              const isExpired = daysLeft < 0;
              const isExpiringSoon = daysLeft >= 0 && daysLeft <= 3;
              return (
                <div key={trial.id} className="glass-card p-5 group">
                  <div className="flex items-center gap-3 mb-3">
                    <span className="text-2xl">{trial.icon}</span>
                    <div className="flex-1"><p className="text-sm font-semibold text-gray-100">{trial.name}</p><p className="text-xs text-gray-400">{formatDateShort(trial.start_date)} - {formatDateShort(trial.end_date)}</p></div>
                    <div className="flex items-center gap-2">
                      <span className={`text-xs font-medium px-2 py-1 rounded-full ${isExpired ? "bg-expense/20 text-expense" : isExpiringSoon ? "bg-warning/20 text-warning" : "bg-income/20 text-income"}`}>{isExpired ? "Expired" : `${daysLeft} hari`}</span>
                      <div className="hidden group-hover:flex gap-1">
                        <button onClick={() => startEdit(trial)} className="p-1 hover:bg-white/10 rounded-full"><svg className="w-4 h-4 text-iosBlue" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                        <button onClick={() => { if (confirm("Hapus trial ini?")) deleteTrial(trial.id); }} className="p-1 hover:bg-red-500/20 rounded-full"><svg className="w-4 h-4 text-expense" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                      </div>
                    </div>
                  </div>
                  {trial.amount > 0 && (<div className="flex justify-between items-center pt-3 border-t border-white/10"><span className="text-xs text-gray-400">Harga setelah trial</span><span className="text-sm font-semibold text-gray-100">{formatRupiah(trial.amount)}</span></div>)}
